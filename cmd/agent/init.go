...
func runInit(_ *cobra.Command, args []string) error {
	...
	fmt.Println("  │                                        │")
	fmt.Printf("  │  Agent:       %-24s│\n", fmt.Sprintf("%s %s", agentNameInput, avatarChoice))
	fmt.Printf("  │  Chat:        %-24s│\n", truncate(fmt.Sprintf("%s / %s", info.label, modelInput), 24))
	shieldInfo2 := providers[shieldProvider]
	fmt.Printf("  │  Shield:      %-24s│\n", truncate(fmt.Sprintf("%s / %s", shieldInfo2.label, shieldModel), 24))
	fmt.Printf("  │  Embedding:   %-24s│\n", truncate(embDisplay, 24))
	fmt.Printf("  │  Workspace:   %-24s│\n", truncate(workspacePath, 24))
	fmt.Printf("  │  Web UI:      %-24s│\n", fmt.Sprintf("http://127.0.0.1:%d", webPort))
	fmt.Println("  │                                        │")
	...
	// initClassifier removed — local ONNX classifier deactivated pending
	// model retrain and sidecar migration. See roadmap.
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewMultiSelect[string]().
				...
				Description("Deselect all to skip").
				Options(
...
func writeConfig(path, workspace, agentName, avatar string,
	...
	// Start from the canonical defaults — every field in DefaultConfig() is
	// the single source of truth. The wizard only overrides what it collected.
	defaults := config.DefaultConfig()
	cfg := &defaults
	cfg.Workspace = workspace
	cfg.Identity = types.IdentityConfig{Name: agentName, Avatar: avatar}
	cfg.Roles = types.RolesConfig{Chat: "chat", SubAgent: "chat"}
	cfg.Web.Port = webPort
	cfg.Web.GRPCPort = webPort + registry.GRPCPortOffset
	cfg.Tools.DisabledGroups = []string{"canvas"}
	...
	if sameAsChat {
		...
		cfg.Models = append(cfg.Models, types.ModelEntry{
			...
			Provider:  shieldProv,
			Model:     shieldModel,
			APIKeyEnv: shieldInfo.apiKeyEnv,
			BaseURL:   shieldBaseURL,
		})
		cfg.Roles.Shield = "shield"
	}
...
// FirstMessageTemplate generates the agent's introduction greeting. The
// agent discovers its actual capabilities from the load_tools definition
// sent with InitialToolDefs — that's the single source of truth. This
// template is just the greeting; the LLM describes what it can do based
// on what load_tools actually offers.
func FirstMessageTemplate(agentName string, _ *types.AgentConfig) string {
	return fmt.Sprintf("Hey! I'm %s, your personal AI agent. "+
		"I'm running on your machine, secured by Shield, and everything stays local. "+
		"What would you like to work on?", agentName)
}
