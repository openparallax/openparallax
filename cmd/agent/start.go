...
func init() {
	...
	startCmd.Flags().BoolVar(&startWeb, "web", false, "open the web UI in a browser on startup")
...
func runStart(_ *cobra.Command, args []string) error {
	...
	pm := newProcessManager(cfgPath, startVerbose, cfg.Agents.CrashRestartBudget, cfg.Agents.CrashWindowSeconds)
	...
	if webStatus.ok {
		webURL := fmt.Sprintf("http://127.0.0.1:%d", webStatus.port)
...
func newProcessManager(configPath string, verbose bool, crashBudget, crashWindowS int) *processManager {
	if crashBudget <= 0 {
		crashBudget = 5
	}
	if crashWindowS <= 0 {
...
func (pm *processManager) spawnEngine(ctx context.Context) (int, webStatus, error) {
	...
	if startDaemon {
		platform.ApplyDaemonProcAttr(pm.cmd)
	}
...
func (pm *processManager) monitor(ctx context.Context) {
	...
	for {
		...
		cutoff := now.Add(-time.Duration(pm.crashWindowS) * time.Second)
		recentCrashes := 0
		...
		if recentCrashes >= pm.crashBudget {
			fmt.Fprintf(os.Stderr, "Engine has crashed %d times in %d seconds. Giving up.\n", recentCrashes, pm.crashWindowS)
			return
		}
