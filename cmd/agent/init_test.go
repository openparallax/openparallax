...
func TestFirstMessageTemplate(t *testing.T) {
	...
	assert.Contains(t, msg, "secured by Shield")
	assert.Contains(t, msg, "What would you like to work on?")
}

func TestFirstMessageTemplateWithDifferentName(t *testing.T) {
	cfg := &types.AgentConfig{}
	msg := FirstMessageTemplate("Atlas", cfg)
	assert.Contains(t, msg, "I'm Atlas")
}
