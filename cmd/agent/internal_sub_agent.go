...
func runInternalSubAgent(_ *cobra.Command, _ []string) error {
	...
	// Load workspace config for values not carried over gRPC.
	subCfg := config.DefaultConfig()
	cfgPath := filepath.Join(subAgentWorkspace, "config.yaml")
	...
	cfg := agent.LoopConfig{
		...
		MaxRounds:     maxCalls,
		ContextWindow: subCfg.Agents.ContextWindow,
	}
