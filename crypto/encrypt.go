...
// hkdfSalt is a static application-specific salt for HKDF key derivation.
// Not secret — provides domain separation per RFC 5869 Section 3.1.
var hkdfSalt = []byte("openparallax-hkdf-v1")
...
func DeriveKey(canaryHex, info string) ([]byte, error) {
	...
	reader := hkdf.New(sha256.New, raw, hkdfSalt, []byte(info))
