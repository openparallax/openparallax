...
func (r *anthropicToolStreamReader) Next() (StreamEvent, error) {
	...
	for r.stream.Next() {
		...
		case "message_start":
			...
			// Input/cache tokens come from message_start; output tokens come from
			// message_delta (which carries the total for this round).
		...
		case "message_delta":
			...
			if md.Usage.OutputTokens > 0 {
				// message_delta carries the total output tokens for this round.
				// Accumulate across continuation rounds.
...
// reach Anthropic on the very next request.
func (r *anthropicToolStreamReader) SetTools(tools []ToolDefinition) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.tools = tools
}
