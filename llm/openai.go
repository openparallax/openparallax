...
// toolCallIDRe matches any character NOT in the Anthropic tool_use_id
// allowlist. Used to sanitize IDs from proxies that may pass through
// characters (dots, colons, slashes) that Anthropic rejects.
var toolCallIDRe = regexp.MustCompile(`[^a-zA-Z0-9_-]`)

// sanitizeToolCallID replaces characters outside [a-zA-Z0-9_-] with
// underscores so the ID is safe for any downstream provider. The
// Anthropic API enforces ^[a-zA-Z0-9_-]+$ on tool_use_id and
// tool_result.tool_use_id; OpenAI-compatible proxies that translate
// to Anthropic inherit this constraint.
func sanitizeToolCallID(id string) string {
	return toolCallIDRe.ReplaceAllString(id, "_")
}
...
func (r *openaiToolStreamReader) Next() (StreamEvent, error) {
	...
	for r.stream.Next() {
		...
		// Accumulate across continuation rounds so multi-turn totals are correct.
		...
		for _, tc := range choice.Delta.ToolCalls {
			...
			if tc.ID != "" {
				existing.id = sanitizeToolCallID(tc.ID)
			}
			if tc.Function.Name != "" {
			...
			if existing.id == "" && existing.name != "" {
				existing.id = fmt.Sprintf("call_%s_%d", sanitizeToolCallID(existing.name), idx)
			}
		}
...
// SetTools replaces the captured tool list. The next continuation call
// (SendToolResults) will use the new set, so dynamically loaded tools
// reach the provider's function-call schema on the very next request.
func (r *openaiToolStreamReader) SetTools(tools []ToolDefinition) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.tools = tools
}
