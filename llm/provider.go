...
// maxResponseBuf is the safety cap on accumulated response text. 10MB of text
// is ~2.5M tokens — no legitimate response reaches this. The stream continues
// delivering tokens to the client; only the internal buffer (FullText()) is capped.
const maxResponseBuf = 10 * 1024 * 1024
...
type ToolStreamReader interface {
	...
	// SetTools replaces the active tool definitions used for the next
	// continuation call. Used when the LLM dynamically loads additional
	// tool groups via the load_tools meta-tool — the new definitions must
	// reach the provider on the very next request, otherwise the LLM
	// gets told "loaded" but the function-call schema is unchanged and
	// the freshly loaded tools are not actually callable.
