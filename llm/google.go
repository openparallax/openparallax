...
func (r *geminiToolStreamReader) Next() (StreamEvent, error) {
	...
	// Capture usage from streaming chunks. Gemini sends UsageMetadata on
	// each chunk; later chunks carry cumulative totals for the current round.
	// We track the latest per-round values and combine with previous rounds
	// in Usage().
	if resp != nil && resp.UsageMetadata != nil {
		r.roundInput = int(resp.UsageMetadata.PromptTokenCount)
		r.roundOutput = int(resp.UsageMetadata.CandidatesTokenCount)
	}
	...
	for _, part := range resp.Candidates[0].Content.Parts {
		...
		case genai.FunctionCall:
			...
			r.callIndex++
			tc := ToolCall{ID: fmt.Sprintf("call_google_%d", r.callIndex), Name: p.Name, Arguments: args}
			return StreamEvent{Type: EventToolCallComplete, ToolCall: &tc}, nil
		}
	}
...
// SetTools replaces the captured tool list. Used by the dynamic
// load_tools meta-tool path so newly loaded definitions reach the
// provider on the next continuation call.
func (r *geminiToolStreamReader) SetTools(tools []ToolDefinition) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.tools = tools
}
...
func (r *geminiToolStreamReader) SendToolResults(results []ToolResult) error {
	...
	// Fold the current round's usage into the cumulative total before
	// starting a new stream. Next() will overwrite roundInput/roundOutput
	// with the new round's values.
	r.usage.InputTokens += r.roundInput
	r.usage.OutputTokens += r.roundOutput
	r.roundInput = 0
	r.roundOutput = 0
...
// Usage returns accumulated token usage across all continuation rounds.
func (r *geminiToolStreamReader) Usage() TokenUsage {
	r.mu.Lock()
	defer r.mu.Unlock()
	return TokenUsage{
		InputTokens:  r.usage.InputTokens + r.roundInput,
		OutputTokens: r.usage.OutputTokens + r.roundOutput,
	}
}
