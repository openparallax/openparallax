...
func crossPlatformRules() []HeuristicRule {
	return []HeuristicRule{
		...
		{ID: "XP-020", Name: "find_delete", Pattern: `(?i)find\s+.*-delete`, Category: "shell_injection", Severity: "critical", Description: "find with -delete flag can recursively remove files across the entire filesystem."},
		...
		{ID: "XP-022", Name: "sql_drop", Pattern: `(?i)(DROP\s+(TABLE|DATABASE|SCHEMA|INDEX)|TRUNCATE\s+)`, Category: "shell_injection", Severity: "critical", Description: "SQL destructive operation — drops or truncates database objects."},
		...
		{ID: "XP-024", Name: "systemctl_disable", Pattern: `(?i)systemctl\s+(stop|disable|mask)\s`, Category: "shell_injection", Severity: "high", Description: "Stopping or disabling system services can disrupt operations."},
		...
		{ID: "XP-026", Name: "copy_system_file", Pattern: `(?i)(cp|copy)\s+/(etc|var/log|proc)/\S+\s+/(tmp|dev/shm|home)`, Category: "shell_injection", Severity: "high", Description: "Copying system files to a user-accessible location may expose sensitive data."},
		{ID: "XP-027", Name: "malicious_package_install", Pattern: `(?i)(pip|pip3|npm|gem|cargo)\s+install\s+.*--index-url\s+https?://`, Category: "shell_injection", Severity: "critical", Description: "Package installation from a custom registry — possible supply chain attack."},
		...
		{ID: "XP-030", Name: "curl_pipe_git", Pattern: `(?i)(curl|wget)\s+.*\|\s*git\s`, Category: "shell_injection", Severity: "high", Description: "Piping downloaded content to git — could apply malicious patches."},
		{ID: "XP-031", Name: "curl_post_file", Pattern: `(?i)curl\s+.*-d\s+@`, Category: "shell_injection", Severity: "high", Description: "curl POST with file content — possible data exfiltration."},
		{ID: "XP-032", Name: "git_force_push_main", Pattern: `(?i)git\s+push\s+--force\s+.*main`, Category: "shell_injection", Severity: "critical", Description: "Force push to main can destroy shared branch history."},
