<script lang="ts">
  ...
  import { sessions, currentSessionId, currentMode, scrollToMessageId, suppressAutoScroll } from '../stores/session';
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (list.length > 0) {
        ...
        // First start — auto-create a session so the UI is immediately usable.
        const sess = await createSession('normal');
        sessions.set([sess]);
  ...
  async function switchToSession(id: string, messageId?: string) {
    ...
    // Set the scroll target BEFORE changing the session so the
    // ChatPanel reactive guard ($scrollToMessageId check) sees it
    // and skips the pinToBottom=true reset. The actual scroll
    // happens in afterUpdate after loadMessages renders the DOM.
    if (messageId) {
...
<div class="sidebar glass" class:mobile-open={$sidebarOpen}>
  ...
  {#if searchResults !== null}
    <div class="search-results">
      ...
      {:else}
        {#each searchResults as result (result.message_id || result.session_id)}
