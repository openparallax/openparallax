<script lang="ts">
  ...
  function handleContextMenu(e: MouseEvent) {
    ...
    // The context menu uses position:fixed, but backdrop-filter and
    // transform on ancestors create a new containing block that
    // shifts position:fixed from the viewport to the ancestor. To
    // get the correct position regardless of ancestor CSS, we find
    // the element the menu will actually be positioned relative to
    // (the offsetParent chain until we hit the viewport or a
    // containing-block-creating ancestor) by measuring the menu's
    // own parent after render. Instead, use a simpler approach: set
    // coordinates relative to the page and switch to absolute
    // positioning on the message element.
    //
    // Pragmatic: just use the mouse offset relative to the message
    // component root, which is the nearest positioned ancestor.
    const msgEl = (e.currentTarget as HTMLElement);
    const rect = msgEl.getBoundingClientRect();
    const menuW = 160;
    const menuH = 40;
    menuX = Math.min(e.clientX - rect.left, rect.width - menuW);
    menuY = e.clientY - rect.top;
    if (menuX < 0) menuX = 0;
    if (menuY + menuH > window.innerHeight - rect.top) {
