...
**Caveat on Tier 1:** ~2s P50 is dominated by the ONNX classifier's per-call overhead in our local in-process pure-Go implementation. The future CGo classifier sidecar will bring this to ~30ms (50-100x speedup). Tier 2 being only ~18ms slower than Tier 1 (2002 vs 1984) is a sign that the pure-Go runtime is the bottleneck, not the LLM call.
