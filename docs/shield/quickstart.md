...
The in-process ONNX classifier has been deactivated. Tier 1 runs heuristic-only (79 rules) by default. A retrained model and CGo sidecar binary are the [immediate next priority](/project/roadmap#immediate-next-steps). When the sidecar ships, enable it with `classifier_mode: sidecar` and `classifier_addr`.
