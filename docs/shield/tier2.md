---
description: Shield Tier 2 — independent LLM evaluator with canary token injection detection. Provides contextual reasoning and detects evaluator compromise attempts.
---
...
The prompt contains a `{{CANARY_TOKEN}}` placeholder that is replaced with the actual workspace canary token at engine startup. This token is a random string that attackers cannot predict.
