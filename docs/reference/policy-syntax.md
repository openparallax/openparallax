...
Policies live in the workspace's `policies/` directory. The active policy is specified in `config.yaml`:
...
|--------|------|-----------|
| **Default** | `policies/default.yaml` | Balanced — allows reads, requires evaluation for writes and commands |
| **Permissive** | `policies/permissive.yaml` | Minimal friction — allows most actions, blocks only dangerous paths |
| **Strict** | `policies/strict.yaml` | Maximum security — requires Tier 2 for any write or command |
