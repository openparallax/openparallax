...
| File | Purpose |
|---|---|
| `internal/llm/provider.go` | Provider interface, StreamReader, ToolStreamReader, all types, options |
| `internal/llm/factory.go` | NewProvider factory, APIHost, TestConnection |
| `internal/llm/anthropic.go` | Anthropic Claude implementation |
| `internal/llm/openai.go` | OpenAI (and compatible) implementation |
| `internal/llm/google.go` | Google Gemini implementation |
| `internal/llm/ollama.go` | Ollama implementation |
| `internal/types/config.go` | LLMConfig struct |
