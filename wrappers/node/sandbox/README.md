# @openparallax/sandbox

Kernel-level process isolation (Landlock, sandbox-exec, Job Objects)

TypeScript wrapper for the [OpenParallax](https://docs.openparallax.dev) sandbox module. Communicates with a pre-built Go binary over JSON-RPC (stdin/stdout).

## Installation

```bash
npm install @openparallax/sandbox
```

## Documentation

See the [sandbox documentation](https://docs.openparallax.dev/sandbox/) for the full API reference and examples.

## License

Apache License 2.0
