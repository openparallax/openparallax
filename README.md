...
Each tier has a different failure mode. An attacker must simultaneously evade pattern matching, jailbreak a separate LLM that treats everything as data, AND convince the user — for a single action. All tiers fail closed. ML classification via sidecar is the [immediate next priority](https://docs.openparallax.dev/project/roadmap#immediate-next-steps).
...
| **[Memory](https://docs.openparallax.dev/memory/)** | Semantic memory with pluggable backends (SQLite default; pgvector, Qdrant, Pinecone planned) | &#10003; | planned | planned |
...
| **[Channels](https://docs.openparallax.dev/channels/)** | WhatsApp, Telegram, Discord, Signal, iMessage (Slack and Teams planned) | &#10003; | planned | planned |
...
| **[IFC](https://docs.openparallax.dev/security/ifc)** | Information flow control with sensitivity labels | &#10003; | | |
...
- **[API Reference](https://docs.openparallax.dev/reference/config)** — config schema, event types, 69 action types, REST endpoints, WebSocket protocol
- **[Roadmap](https://docs.openparallax.dev/project/roadmap)** — all planned features, from memory backends to A2A inter-agent collaboration
