...
Always run terraform plan before apply.`
	...
	skill, err := parseSkill(content, "deploy-guide")
...
func TestParseSkillNoFrontmatter(t *testing.T) {
	_, err := parseSkill("No frontmatter here", "test")
	assert.Error(t, err)
}
...
func TestParseSkillBadNameFormat(t *testing.T) {
	cases := []struct {
		name string
		dir  string
	}{
