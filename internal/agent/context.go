...
When you learn a durable fact about the user (name, preferences, role, projects, recurring contacts, working hours), append it to USER.md (a workspace file) so future sessions inherit it. Don't store ephemera or one-off task details.
Before guessing OS, paths, username, home directory, or shell, check the Host Environment block in USER.md. If it is not set, call system_info from the system tool group to detect it and append the result to USER.md before proceeding.
