...
const (
	// rateLimitUnauth is the per-IP request rate for unauthenticated clients.
	rateLimitUnauth = 10
	// rateLimitAuth is the per-IP request rate for authenticated clients.
	rateLimitAuth = 60
)
...
func withRateLimit(next http.Handler, rl *rateLimiter, auth *authConfig) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		ip := extractIP(r)
		limit := float64(rateLimitUnauth) // unauthenticated: 10 req/min

		if auth != nil {
			cookie, err := r.Cookie("op_session")
			if err == nil && cookie.Value == auth.sessionToken {
				limit = rateLimitAuth // authenticated: 60 req/min
			}
		} else {
