...
func (s *Server) handleAudit(w http.ResponseWriter, r *http.Request) {
	...
	// Server-side filtering — additive to existing client-side filtering.
	if filterType := r.URL.Query().Get("event_type"); filterType != "" {
		filterTypeFloat, _ := strconv.ParseFloat(filterType, 64)
		var filtered []map[string]any
		for _, entry := range allEntries {
			if et, ok := entry["event_type"].(float64); ok && et == filterTypeFloat {
				filtered = append(filtered, entry)
			}
		}
		allEntries = filtered
	}
	if filterSession := r.URL.Query().Get("session_id"); filterSession != "" {
		var filtered []map[string]any
		for _, entry := range allEntries {
			if sid, ok := entry["session_id"].(string); ok && sid == filterSession {
				filtered = append(filtered, entry)
			}
		}
		allEntries = filtered
