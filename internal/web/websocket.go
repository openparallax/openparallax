...
const (
	// wsMaxMessageBytes is the maximum size of a single WebSocket message (10 MB).
	wsMaxMessageBytes = 10 * 1024 * 1024
	// scannerBufSize is the buffer size for line scanners reading log/audit files.
...
// Uses Decide (not DecideForSession) because the user is already
// authenticated on the WebSocket — session affinity adds no security
// value and breaks cross-session approval workflows.
