...
// Returns the full response text. Blocks until the pipeline emits
// response_complete or the context is cancelled.
func (m *Manager) HandleMessage(ctx context.Context, adapterName, chatID, content string, mode types.SessionMode) (string, error) {
	...
	// ProcessMessageForWeb blocks on <-ctx.Done(), so run it in a goroutine.
	// We cancel its context once the response is collected.
	pipeCtx, pipeCancel := context.WithCancel(ctx)
	defer pipeCancel()

	errCh := make(chan error, 1)
	go func() {
		errCh <- m.engine.ProcessMessageForWeb(pipeCtx, collector, sessionID, messageID, content, mode)
	}()
	...
	case <-ctx.Done():
		return "", ctx.Err()
	case err := <-errCh:
		// ProcessMessageForWeb returned early (e.g. agent unavailable).
		if err != nil {
			return "", err
		}
		return collector.fullResponse(), nil
...
func (m *Manager) getOrCreateSession(adapterName, chatID string, mode types.SessionMode) string {
	...
	sid := crypto.NewID()
	if existing, loaded := m.sessions.LoadOrStore(key, sid); loaded {
		s, _ := existing.(string)
		return s
	}
