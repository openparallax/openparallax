...
type Adapter struct {
	...
	activeNumbers  map[string]bool
	activeNumsMu   sync.Mutex
...
func New(cfg *types.WhatsAppConfig, manager *channels.Manager, log *logging.Logger) *Adapter {
	...
	return &Adapter{
		...
		activeNumbers:  make(map[string]bool),
		manager:        manager,
		log:            log,
...
func (a *Adapter) handleWebhook(ctx context.Context) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if a.appSecret != "" {
			...
			if !verifyWebhookSignature(body, sig, a.appSecret) {
...
func (a *Adapter) processMessage(ctx context.Context, msg whatsappMessage) {
	...
	a.activeNumsMu.Lock()
	a.activeNumbers[from] = true
	a.activeNumsMu.Unlock()
...
// RequestApproval sends a Tier 3 approval prompt via WhatsApp. WhatsApp does
// not support inline buttons for non-template messages, so the message includes
// text instructions to reply with "approve" or "deny".
func (a *Adapter) RequestApproval(actionID, toolName, reasoning string, timeoutSecs int) error {
	a.activeNumsMu.Lock()
	numbers := make([]string, 0, len(a.activeNumbers))
	for num := range a.activeNumbers {
		numbers = append(numbers, num)
	}
	a.activeNumsMu.Unlock()

	if len(numbers) == 0 {
		return fmt.Errorf("no active numbers to send approval request")
	}
	...
	for _, num := range numbers {
		_ = a.SendMessage(num, &channels.ChannelMessage{Text: text})
	}
	return nil
}
