...
// validateProvider rejects provider names not in the supported set.
func validateProvider(_ *types.AgentConfig, value string) error {
	if !validProviders[value] {
		return fmt.Errorf("provider must be one of: anthropic, openai, google, ollama (got %q)", value)
	}
	return nil
}
