...
func (f *FileFormatExecutor) archiveCreate(action *types.ActionRequest) *types.ActionResult {
	rawOutput := firstNonEmptyString(action.Payload, "output", "destination", "path")
	if rawOutput == "" {
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: "output archive path is required (pass it as the 'output' field)"}
	}
	output, outputErr := ResolveInWorkspace(rawOutput, f.workspacePath)
	if outputErr != nil {
		return ErrorResult(action.RequestID, fmt.Sprintf("output path: %s", outputErr), "path outside workspace")
	}
	...
	for _, p := range pathsRaw {
		if s, ok := p.(string); ok {
			resolved, resolveErr := ResolveInWorkspace(s, f.workspacePath)
			if resolveErr != nil {
				return ErrorResult(action.RequestID, fmt.Sprintf("source path: %s", resolveErr), "path outside workspace")
			}
	...
	default:
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: fmt.Sprintf("unsupported archive format for %q (supported: .zip, .tar.gz, .tgz)", output)}
	}
...
func (f *FileFormatExecutor) archiveExtract(action *types.ActionRequest) *types.ActionResult {
	// Accept "archive" (canonical) and "path" / "source" / "file" as
	// fallback keys. The schema documents "archive" but real LLMs guess
	// the natural alternatives, and silently resolving a missing key to
	// the workspace root produces the misleading "unsupported format"
	// error rather than the actual cause.
	rawArchive := firstNonEmptyString(action.Payload, "archive", "path", "source", "file")
	if rawArchive == "" {
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: "archive path is required (pass it as the 'archive' field)"}
	}
	archive, archiveErr := ResolveInWorkspace(rawArchive, f.workspacePath)
	if archiveErr != nil {
		return ErrorResult(action.RequestID, fmt.Sprintf("archive path: %s", archiveErr), "path outside workspace")
	}
	dest := f.workspacePath
	if d, ok := action.Payload["destination"].(string); ok && d != "" {
		var destErr error
		dest, destErr = ResolveInWorkspace(d, f.workspacePath)
		if destErr != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("destination path: %s", destErr), "path outside workspace")
		}
	}
	...
	default:
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: fmt.Sprintf("unsupported archive format for %q (supported: .zip, .tar.gz, .tgz)", archive)}
	}
...
func extractZip(archive, dest string) (int, error) {
	...
	for _, zf := range r.File {
		...
		if totalBytes > maxArchiveSize {
			return count, fmt.Errorf("archive extraction exceeded %d byte cumulative limit", maxArchiveSize)
		}
...
func extractTarGz(archive, dest string) (int, error) {
	...
	for {
		...
		case tar.TypeSymlink, tar.TypeLink:
			return count, fmt.Errorf("archive contains unsupported entry type (symlink/hardlink): %s", header.Name)
...
func (f *FileFormatExecutor) pdfRead(action *types.ActionRequest) *types.ActionResult {
	rawPath := firstNonEmptyString(action.Payload, "path", "file", "source")
	if rawPath == "" {
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: "PDF path is required (pass it as the 'path' field)"}
	}
	path, pathErr := ResolveInWorkspace(rawPath, f.workspacePath)
	if pathErr != nil {
		return ErrorResult(action.RequestID, fmt.Sprintf("PDF path: %s", pathErr), "path outside workspace")
	}
...
func (f *FileFormatExecutor) spreadsheetRead(action *types.ActionRequest) *types.ActionResult {
	rawPath := firstNonEmptyString(action.Payload, "path", "file", "source")
	if rawPath == "" {
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: "spreadsheet path is required (pass it as the 'path' field)"}
	}
	path, pathErr := ResolveInWorkspace(rawPath, f.workspacePath)
	if pathErr != nil {
		return ErrorResult(action.RequestID, fmt.Sprintf("spreadsheet path: %s", pathErr), "path outside workspace")
	}
	maxRows := 100
...
func (f *FileFormatExecutor) spreadsheetWrite(action *types.ActionRequest) *types.ActionResult {
	rawPath := firstNonEmptyString(action.Payload, "path", "output", "destination", "file")
	if rawPath == "" {
		return &types.ActionResult{RequestID: action.RequestID, Success: false, Error: "spreadsheet path is required (pass it as the 'path' field)"}
	}
	path := ResolvePath(rawPath, f.workspacePath)
