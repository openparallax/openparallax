...
func NewRegistry(workspacePath string, cfg *types.AgentConfig, oauthMgr *oauth.Manager, log *logging.Logger) *Registry {
	...
	// Resolve agent config for executor timeouts.
	var agentsCfg types.AgentsConfig
	if cfg != nil {
	...
	r.register(NewShellExecutor(agentsCfg.ShellTimeoutSeconds))
	...
	// system_info is always available (it uses Go stdlib only). The
	// other system tools — clipboard, open, notify, screenshot — are
	// gated per-tool inside NewSystemExecutor based on which platform
	// binaries are installed. Tools whose binary is missing are simply
	// not advertised to the LLM.
	if system := NewSystemExecutor(workspacePath); system != nil {
		r.register(system)
		if log != nil {
			log.Info("executor_registered", "executor", "system",
				"tools", len(system.SupportedActions()))
		}
	}

	if browser := NewBrowserExecutor(log, BrowserConfig{
		NavTimeoutSec:  agentsCfg.BrowserNavTimeoutSeconds,
		IdleMinutes:    agentsCfg.BrowserIdleMinutes,
		MaxNavFailures: agentsCfg.MaxConsecutiveNavFailures,
	}); browser != nil {
		r.register(browser)
	}
