...
// validateGitRef rejects refs that could be interpreted as flags or contain
// shell-unsafe characters. The key defense is rejecting leading dashes to
// prevent --upload-pack style option injection.
func validateGitRef(ref string) error {
	if ref == "" {
		return fmt.Errorf("ref is empty")
	}
	if strings.HasPrefix(ref, "-") {
		return fmt.Errorf("ref must not start with a dash")
	}
	if !safeGitRef.MatchString(ref) {
...
func (g *GitExecutor) diff(action *types.ActionRequest, repoPath string) *types.ActionResult {
	...
	if commit, ok := action.Payload["commit"].(string); ok && commit != "" {
		if err := validateGitRef(commit); err != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("invalid commit ref: %s", err), "invalid git ref")
		}
...
func (g *GitExecutor) log(action *types.ActionRequest, repoPath string) *types.ActionResult {
	...
	if branch, ok := action.Payload["branch"].(string); ok && branch != "" {
		if err := validateGitRef(branch); err != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("invalid branch ref: %s", err), "invalid git ref")
		}
...
func (g *GitExecutor) push(action *types.ActionRequest, repoPath string) *types.ActionResult {
	...
	if r, ok := action.Payload["remote"].(string); ok && r != "" {
		if err := validateGitRef(r); err != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("invalid remote: %s", err), "invalid git ref")
		}
		remote = r
	}
	...
	if branch, ok := action.Payload["branch"].(string); ok && branch != "" {
		if err := validateGitRef(branch); err != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("invalid branch: %s", err), "invalid git ref")
		}
		args = append(args, branch)
	}
	return g.runGit(action.RequestID, repoPath, args...)
...
func (g *GitExecutor) branch(action *types.ActionRequest, repoPath string) *types.ActionResult {
	...
	case "create":
		...
		if err := validateGitRef(name); err != nil {
			return ErrorResult(action.RequestID, fmt.Sprintf("invalid branch name: %s", err), "invalid git ref")
		}
...
func (g *GitExecutor) checkout(action *types.ActionRequest, repoPath string) *types.ActionResult {
	...
	if err := validateGitRef(ref); err != nil {
		return ErrorResult(action.RequestID, fmt.Sprintf("invalid ref: %s", err), "invalid git ref")
	}
