...
// Default browser configuration values, used when no config override is provided.
const (
	defaultBrowserNavTimeout      = 30 * time.Second
	defaultBrowserIdleTimeout     = 5 * time.Minute
	defaultMaxConsecutiveNavFails = 3
	defaultBrowserSubtaskTimeout  = 15 * time.Second
	// browserMaxContentChars is the max characters returned from a page
	// extraction before truncation.
	browserMaxContentChars = 50000
)
...
type BrowserExecutor struct {
	...
	lastUsed            time.Time
	running             bool
	consecutiveNavFails int
}

// BrowserConfig carries the configurable browser parameters from config.yaml.
type BrowserConfig struct {
	NavTimeoutSec  int // 0 = default 30
	IdleMinutes    int // 0 = default 5
	MaxNavFailures int // 0 = default 3
}
...
func NewBrowserExecutor(log *logging.Logger, bcfg BrowserConfig) *BrowserExecutor {
	...
	if bcfg.NavTimeoutSec > 0 {
		navTimeout = time.Duration(bcfg.NavTimeoutSec) * time.Second
	}
	idleTimeout := defaultBrowserIdleTimeout
	if bcfg.IdleMinutes > 0 {
		idleTimeout = time.Duration(bcfg.IdleMinutes) * time.Minute
	}
	maxFails := defaultMaxConsecutiveNavFails
	if bcfg.MaxNavFailures > 0 {
		maxFails = bcfg.MaxNavFailures
	}
...
func (b *BrowserExecutor) navigate(ctx context.Context, action *types.ActionRequest) *types.ActionResult {
	...
	// After b.maxNavFails consecutive navigation failures,
	// stop trying — the browser is fundamentally broken for this session
	// (e.g. Flatpak sandbox, headless misconfiguration). Tell the LLM
	// plainly so it can fall back to other tools or training knowledge
	// instead of burning round-trips on retries.
	b.mu.Lock()
	fails := b.consecutiveNavFails
	b.mu.Unlock()
	if fails >= b.maxNavFails {
		return ErrorResult(action.RequestID,
			fmt.Sprintf("browser navigation disabled after %d consecutive failures — the browser cannot load pages in this environment. Use other tools or your training knowledge instead.", fails),
			"browser disabled")
	}

	var body string
	taskCtx, cancel := context.WithTimeout(ctx, b.navTimeout)
	defer cancel()
	...
	content := truncateContent(text, browserMaxContentChars)
...
func (b *BrowserExecutor) extract(ctx context.Context, action *types.ActionRequest) *types.ActionResult {
	...
	taskCtx, cancel := context.WithTimeout(ctx, defaultBrowserSubtaskTimeout)
	defer cancel()
	...
	content = truncateContent(content, browserMaxContentChars)
