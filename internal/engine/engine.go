...
import (
	...
	"github.com/openparallax/openparallax/internal/security"
...
type Engine struct {
	...
	agentStreamMu sync.Mutex // protects agentStream.Send() calls
	currentMsgOTR bool
	...
	// summarizedAt tracks the message count per session at the last
	// successful summarization. Used to avoid re-summarizing the same
...
func New(configPath string, verbose bool) (*Engine, error) {
	...
	// Seal the non-negotiable security configuration before any subsystem
	// construction. This must be the first operation after config load — it
	// establishes the immutable security posture that all downstream
	// subsystems (Shield, IFC, sandbox, audit, protection layer) respect.
	security.Seal(security.Defaults())
	...
	for _, r := range []struct{ role, name string }{
		{"chat", cfg.Roles.Chat}, {"shield", cfg.Roles.Shield},
		{"embedding", cfg.Roles.Embedding}, {"sub_agent", cfg.Roles.SubAgent},
		{"image", cfg.Roles.Image}, {"video", cfg.Roles.Video},
	} {
		if r.name != "" {
			if err := modelReg.SetRole(r.role, r.name); err != nil {
				log.Warn("role_set_failed", "role", r.role, "error", err)
			}
		}
	}
	...
	shieldPipeline, err := shield.NewPipeline(shield.Config{
		...
		Metrics:             db,  // persists Tier 2 budget across restarts
	})
	if err != nil {
	...
	ag := agent.NewAgent(provider, cfg.Workspace, mem, nil, cfg.Skills.Disabled)
	...
	// Load IFC policy from the workspace's security/ifc/ directory.
	ifcPolicyPath := cfg.Security.IFCPolicy
	if ifcPolicyPath == "" {
		ifcPolicyPath = "security/ifc/default.yaml"
	}
	if !filepath.IsAbs(ifcPolicyPath) {
		ifcPolicyPath = filepath.Join(cfg.Workspace, ifcPolicyPath)
	}
	ifcPolicy, err := ifc.LoadPolicy(ifcPolicyPath)
	if err != nil {
		return nil, fmt.Errorf("ifc policy: %w", err)
	}
	// Apply override_mode from config.yaml if set.
	if cfg.Security.OverrideMode != "" {
		override := ifc.Mode(cfg.Security.OverrideMode)
		if override == ifc.ModeAudit || override == ifc.ModeEnforce {
	...
	// Startup catch-up: summarize any sessions that were never summarized
	// (e.g. because the previous engine crashed or was killed). Runs in
	// the background so it doesn't block engine startup.
	go func() {
		sessions, listErr := db.ListSessions()
		if listErr != nil {
			return
		}
		for _, sess := range sessions {
			eng.summarizeSessionIfNeeded(sess.ID)
		}
	}()
...
func (e *Engine) Start(listenPort ...int) (int, error) {
	...
	go func() {
		if serveErr := e.server.Serve(lis); serveErr != nil {
			e.log.Error("grpc_serve_failed", "error", serveErr)
		}
	}()
...
func (e *Engine) SetupSubAgents(grpcAddr string) {
	maxConcurrent := e.cfg.Agents.MaxConcurrentSubAgents
	if maxConcurrent <= 0 {
		maxConcurrent = 10
	}
	e.subAgentManager = NewSubAgentManager(e, grpcAddr, maxConcurrent)
