...
func setupProtectionWorkspace(t *testing.T) string {
	...
	for _, f := range []string{"config.yaml", "canary.token", "audit.jsonl"} {
	...
	// Create security/shield directory.
	require.NoError(t, os.MkdirAll(filepath.Join(dir, "security/shield"), 0o755))
	require.NoError(t, os.WriteFile(filepath.Join(dir, "security/shield", "default.yaml"), []byte("policy"), 0o644))
...
func TestProtection_ReadPoliciesDir_Blocked(t *testing.T) {
	ws := setupProtectionWorkspace(t)
	action := makeAction(types.ActionReadFile, map[string]any{"path": filepath.Join(ws, "security/shield", "default.yaml")})
	allowed, prot, _ := CheckProtection(action, ws)
	assert.False(t, allowed)
	assert.Equal(t, FullBlock, prot)
}
