...
func crossPlatformRules() []HeuristicRule {
	return []HeuristicRule{
		...
		{ID: "XP-010", Name: "network_fetch_to_exec", Pattern: `(?i)(curl|wget|iwr|irm|Invoke-WebRequest|Invoke-RestMethod)\s+.*\|\s*(sh|bash|python[23]?|perl|ruby|iex|Invoke-Expression|powershell|cmd)`, Category: "shell_injection", Severity: "critical", Description: "Network fetch piped to code execution"},
		{ID: "XP-019", Name: "insecure_transport", Pattern: `(?i)(curl|wget|Invoke-WebRequest|Invoke-RestMethod|iwr|irm)\s+['"]?http://`, Category: "shell_injection", Severity: "high", Description: "Network request over insecure HTTP transport"},
